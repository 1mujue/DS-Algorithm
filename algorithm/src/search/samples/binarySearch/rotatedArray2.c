...
// complexity. However, it is NOT guaranteed that the array does not contain duplicates.
int search(int* nums, int numsSize, int target) {
    ...
    while(left <= right){
        ...
        // in this case, nums[mid] <= nums[left] and nums[mid] >= nums[right].
        // Clearly, there is no way for nums[mid] < nums[left] && nums[mid] > nums[right].
        else if(nums[left] == nums[mid] && nums[right] == nums[mid]){
            left++;
        }
        // in this case, [mid, right] are all equal. 
        else if(nums[left] > nums[mid] && nums[right] == nums[mid]){
            right = mid - 1;
        }
        // in this case, [left, mid] are all equal.
        else if(nums[left] == nums[mid] && nums[right] < nums[mid]){
            left = mid + 1;
        }
    }
